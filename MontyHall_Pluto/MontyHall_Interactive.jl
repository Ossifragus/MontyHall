...
begin
    go
    obs = whichDoor(👈)
    doors_o = repeat(door, outer=(1,3))
    @views doors_o[loc_hand,(👈-1)*p.+(1+mh:p-mh)][nb_hand] = hand[nb_hand]
    @views doors_o[loc_goat,(obs.💁-1)*p.+(1+mg:p-mg)][nb_goat] = goat[nb_goat]
    @views doors_o[loc_host,(obs.💁-1)*p.+(1+mg:p-mg)][nb_host] = host[nb_host]
    if showhost == "show all"
        @views doors_o[loc_car,(obs.🚗-1)*p.+(1+mc:p-mc)][nb_car] = car[nb_car]
    end
    if showhost in ["show host", "show all"] 
        doors_o 
    else
        nothing
    end
...
# ╔═╡ ccad0dac-4b2c-47a8-aa5d-3b90d8256192
md"""
Results with door 3 open:
"""

# ╔═╡ 8328a5b6-3dd1-445c-9f5e-4f8f5d37a303
results[open3]
...
md"""
If you don't switch, the proportion of finding the car is: $(p_not).\
If you switch, the proportion of finding the car is: $(p_switch)
"""
end
